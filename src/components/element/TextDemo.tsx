...
function TextDemo({ locale }: ILocale) {
  ...
  return (
    ...
    >
      <h3 className="bg-code-pattern bg-clip-text bg-top text-3xl font-extrabold text-transparent">
        {t("im")}
      </h3>
      <h3
        className={`${locale === "fa" ? "" : "mt-2"} bg-code-pattern bg-clip-text bg-top font-medium text-transparent`}
        ref={el}
      ></h3>
      {locale === "en" ? null : (
        <h3 className="bg-code-pattern bg-clip-text bg-top text-3xl font-extrabold text-transparent">
